...
def get_top_articles(url, threshold):
    """Parse articles from page and return only those articles with votes greater than or equal to threshold."""
    ...
    # (article title and url is nested within .athing while votes are nested within
    # .athing's sibling, .subtext)
    if articles:
...
def print_articles_sorted(top_articles):
    ...
    for index, item in enumerate(sorted(top_articles, key=lambda k: k.votes, reverse=True)):
        print(f"{index + 1}. {item}\n")


def parse_args():
    parser = argparse.ArgumentParser(
        description='Scrape top articles from Hacker News with at least n votes.')
    parser.add_argument(
        "threshold", help="The minimum number of votes that posts should have.", type=int)
